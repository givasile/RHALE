...
class DALEBinBase(FeatureEffectBase):

    def __init__(self, axis_limits):
        super(DALEBinBase, self).__init__(axis_limits)
    ...
    def plot(
        ...
        if violin:
            limits = self.feature_effect["feature_" + str(feature)]["limits"]
            limits[-1] += 1e-6
            data = self.data
            ind = np.digitize(data[:, feature], limits)
            data_effect = self.data_effect
            data_effect_lst = [data_effect[ind == i + 1, feature] for i in range(len(limits)-1)]
        ...
        vis.ale_plot(
            ...
            title=title,
            violin=violin,
            data_effect_lst=data_effect_lst if violin else None,
        )


class DALE(DALEBinBase):
    def __init__(
        self,
        data: np.ndarray,
        model: callable,
